...
export class ConsentScreen extends Component<PropsType, TermsState> {
    ...
    componentDidMount(): void {
        if (isSVLocale()) this.props.navigation.setOptions({ title: 'Information till studiedeltagare' });
    }
    ...
    handleAgreeClicked = async () => {
        if (isUSLocale()) this.handleUSAgreeClicked();
        if (isGBLocale()) return this.handleUKAgreeClicked();
        if (isSVLocale()) return this.handleSVAgreeClicked();
    };

    canAgree = () => {
        if (isUSLocale()) return this.state.processingChecked && this.state.termsOfUseChecked;
        if (isGBLocale()) return true;
        if (isSVLocale()) return this.state.swedenParticipateChecked && this.state.swedenProcessingChecked;
        return false;
    };
        ...
        return (
            <View style={styles.rootContainer}>
                    <ScrollView>
                        ...
                        <SimpleTextBlock text={[
                            "Projektet kommer att samla in och registrera information om dig via appen Covid-19 Symptom Tracker. Ändamålet med informationsinsamlingen är att möjliggöra forskning på spridningen av covid-19 i den svenska befolkningen. Den rättsliga grunden för informationsinsamlingen är därmed allmänt intresse.",
                            "Appen Covid-19 Symptom Tracker är utvecklad av forskare på King’s College i London, Guy’s Hospital och St Thomas Hospital i London, samt utvecklare på hälsoföretaget Zoe Global Ltd i Storbritannien. Zoe Global Ltd kommer att ansvara för initial insamling av data från Covid-19 Symptom Tracker. Information från de svenska användarna kommer sedan med säker filöverföring och i krypterad form överföras till Lunds Universitet.",
                            "Lunds Universitet kommer sedan att bygga upp en forskningsdatabas med informationen från svenska användare av Covid-19 Symptom Tracker. Forskningsdatabasen kommer att ligga på en säker server utan tillgång för ickeauktoriserad personal eller andra utomstående. Tillgång till data kommer enbart att ges till utvalda forskare i projektet.",
                        ]} />
                        <RegularText>
                            All information som samlas in från användarna av appen Covid-19 Symptom Tracker kommer att sammanställas och analyseras på gruppnivå, och inga individuella karakteristika kommer att kunna identifieras. Resultaten från sammanställningarna och analyserna kommer att skickas löpande till Folkhälsomyndigheten och andra myndigheter och ansvariga inom hälso- och sjukvård. Resultaten kommer också att publiceras löpande på studiens hemsida (<ClickableText onPress={() => Linking.openURL('https://Covid19app.lu.se')}>Covid19app.lu.se</ClickableText>). Vi kommer också att publicera resultat på gruppnivå i medicinska tidskrifter.
                        </RegularText>
                        <SimpleTextBlock text={[
                        ...
                        <RegularText>
                            Enligt EU:s dataskyddsförordning har du rätt att kostnadsfritt få ta del av den information om dig som hanteras i studien, och vid behov få eventuella fel rättade. Du kan också begära att information som du lämnat raderas samt att behandlingen av dina personuppgifter begränsas. Detta kan du göra genom att e-posta till <RegularBoldText>leavecovidtracking@joinzoe.com</RegularBoldText> från den e-postadress du använt för att registrera dig i appen. Inom kort kommer också appen uppdateras och göra det möjligt för dig att göra det här i själva appen. Du kan även kontakta ansvarig forskare i Sverige professor Paul Franks om du har frågor, se kontaktinformation nedan.
                        </RegularText>
                        <RegularText>
                            Det finns också ett dataskyddsombud på Lunds universitet som nås på <RegularBoldText>dataskyddsombud@lu.se</RegularBoldText>. Om du är missnöjd med hur dina uppgifter behandlas har du även rätt att lämna in klagomål till Datainspektionen i Stockholm som är den aktuella tillsynsmyndigheten (<RegularBoldText>datainspektionen@datainspektionen.se</RegularBoldText>). Mer information om detta hittar du via <ClickableText onPress={() => Linking.openURL('https://www.datainspektionen.se/vagledningar/for-dig-som-privatperson/klagomal-och-tips/')}>https://www.datainspektionen.se/vagledningar/for-dig-som-privatperson/klagomal-och-tips/</ClickableText>
                        </RegularText>
                        ...
                        <RegularText>
                            Jag har härmed läst den skriftliga informationen om studien, och jag har haft möjlighet att ställa frågor via epost till den ansvariga forskaren. Om jag vill läsa den skriftliga informationen igen så finns den på <ClickableText onPress={() => Linking.openURL('https://Covid19app.lu.se')}>Covid19app.lu.se</ClickableText>.
                        </RegularText>
                            ...
                            !this.viewOnly && (
                                <View>
                                    <ListItem style={styles.permission}>
                                        ...
                                        <Body style={styles.label}>
                                            <RegularText>
                                                Jag är 18 år eller äldre och jag samtycker till att delta i studien ”Nationellt initiativ för att via en app i realtid kartlägga samhällspridningen av covid-19 i Sverige samt riskfaktorer för att drabbas av en allvarlig sjukdomsbild vid covid-19”.
                                            </RegularText>
                                        </Body>
                                    </ListItem>
                                    <ListItem>
                                        ...
                                        <Body style={styles.label}>
                                            <RegularText>
                                                Jag samtycker till att personuppgifter om mig behandlas på det sätt som beskrivs i forskningspersonsinformationen ovan.
                                            </RegularText>
                                        </Body>
                                    </ListItem>
                                    <ListItem>
                                        ...
                                        <Body style={styles.label}>
                                            <RegularText>
                                                ...
                                                <ClickableText onPress={() => this.props.navigation.navigate('PrivacyPolicySV', {viewOnly: this.viewOnly})}>integritetspolicy</ClickableText>.
                                            </RegularText>
                                        </Body>
                                    </ListItem>
    ...
    renderConsent = () => {
        ...
        if (isUSLocale()) consent = this.renderUSConsent;
        if (isGBLocale()) consent = this.renderUKConsent;
        if (isSVLocale()) consent = this.renderSwedishConsent;
        return consent();
    };
